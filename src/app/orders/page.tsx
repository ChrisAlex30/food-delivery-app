      ...
      <table className="w-full border-separate border-spacing-3">
        ...
        <tbody>

          {
            ...
            <tr key={order.id} className={`text-sm md:text-base ${order.status!=="delivered" && "bg-red-50"}`}>
            ...
            <td className="hidden md:block py-6 px-1">{order.products[0].title}</td>
            {
              session?.user.isAdmin?(
                ...
                <form className="flex items-center justify-center gap-4" onSubmit={(e)=>handleUpdate(e,order.id)}>
                     <input placeholder={order.status} type="text" className="p-2 ring-1 ring-red-100 rounded-md" />
